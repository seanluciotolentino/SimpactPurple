...
The figures and subplots are:
    ...
    -Age-disparate relationships (2 sex x 2 years = 4 subplots)
    -Non-age-disparate relationships (2 sex x 2 years = 4 subplots)
...
def load_data():
    ...
    """
    files = os.popen('ls ABCoutput/ABC*').read().split('\n')[:-1]
    if 'ABCoutput/ABCtogether.csv' in files:
        return np.loadtxt('ABCoutput/ABCtogether.csv', delimiter=",")
    else:
        data = np.loadtxt(open(files[0]), delimiter=",")
        
        for filename in files[1:]:
            data = np.append(data, np.loadtxt(open(filename), delimiter=","),axis=0)
        np.savetxt(fname='ABCoutput/ABCtogether.csv',X=data,delimiter=",")
        return data
...
def make_age_disparate_figure(data, lowerbound, upperbound, rows, title):
    plt.figure(figsize=(10,10))
    plt.suptitle(title,fontsize=20)
...
threshold = 250
red_marker_location = 0.24  # for showing survey data
ylimit = 0.26  # upper y-lim for graphs
col = 'b'  # color of bars
save = True  # False #save to file?

data = load_data()
#accepted = data[data[:,-1]<=threshold,:]
accepted = data[np.random.random((len(data),1))[:,0]<=0.16,:]
if len(accepted) <= 0:
    raise ValueError, "No simulations accepted with threshold value of "+str(threshold)
else:
    print len(data),"simulations |",len(accepted),"accepted |", float(len(accepted))/len(data),"percent"
...
if save:
    plt.savefig('ABCgraphs/posterior_'+col+'.png', dpi = 150)

#%% Distribution of distances
plt.figure()
hist = plt.hist(data[:,-1], bins = range(0,1000,25), color = col)
...
if save:
    plt.savefig('ABCgraphs/distance_distribution_'+col+'.png', dpi = 150)
...
if save:
    plt.savefig('ABCgraphs/mp_young_'+col+'.png', dpi = 150)
...
if save:
    plt.savefig('ABCgraphs/mp_adult_'+col+'.png', dpi = 150)
...
rows = [8, 10, 12, 14]
make_age_disparate_figure(survey_data, lb, ub, rows, "Non-age-disparate Relationships")
if save:
    plt.savefig('ABCgraphs/non_ad_'+col+'.png', dpi = 150)
...
rows = [9, 11, 13, 15]
make_age_disparate_figure(survey_data, lb, ub, rows, "Age-disparate Relationships")
if save:
    plt.savefig('ABCgraphs/ad_'+col+'.png', dpi = 150)